...
GetActivePcrBanks(EFI_TCG2_BOOT_SERVICE_CAPABILITY *TpmCapability)
{
	...
	if (TpmCapability->StructureVersion.Major == 1 &&
		...
		Banks = Tpm1Capability->HashAlgorithmBitmap & EFI_TCG_BOOT_HASH_ALG_SHA1;
...
STATIC VOID
ShowEventData(TCG_PCR_EVENT *Event)
{
	Print(L"  Event Data:\n");

	if (Event->EventType == EV_POST_CODE ||
	    Event->EventType == EV_S_CRTM_VERSION ||
	    Event->EventType == EV_SEPARATOR) {
		Print(L"    ");
		ShowHexString(Event->Event, Event->EventSize);
		Print(L"\n");
	} else if (Event->EventType == EV_EFI_VARIABLE_DRIVER_CONFIG ||
		   Event->EventType == EV_EFI_VARIABLE_BOOT)
		ShowVariableLog(Event->Event, Event->EventSize);
	else if (Event->EventType == EV_EFI_BOOT_SERVICES_APPLICATION ||
		   Event->EventType == EV_EFI_BOOT_SERVICES_DRIVER ||
		   Event->EventType == EV_EFI_RUNTIME_SERVICES_DRIVER)
		ShowImageLog(Event->Event, Event->EventSize);
	else {
		Print(L"    ");
		ShowHexString(Event->Event, Event->EventSize);
		Print(L"\n");
